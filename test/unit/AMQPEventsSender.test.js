...
describe('AMQPEventsSender', () => {
  ...
  beforeEach(() => {
    ...
    channelStub = {
      ...
      on: sinon.stub().returns(channelStub)
    };
    connectionStub = {
  ...
  it('should be constructable', () => {
    ...
    expect(sender._channelPromise).to.be.equal(null);
  });
  ...
  describe('#send', () => {
    ...
    it('should create channel', async () => {
      const sender = new AMQPEventsSender(connectionStub, queueName);
      sinon.stub(sender, '_initChannel').returns(Promise.resolve(channelStub));
      expect(sender._initChannel).not.to.have.been.called;
      await sender.send({
        key: 'some-value'
      });
      expect(sender._initChannel).to.have.been.calledOnce;
    });
  ...
  describe('#_initChannel', () => {
    ...
    it('should be caching and does not create channel if it exists', async () => {
      const sender = new AMQPEventsSender(connectionStub, queueName);
      sinon.stub(sender, '_subscribeToChannel');
      const channel = await sender._initChannel();
      expect(connectionStub.createChannel).to.have.been.calledOnce;
      expect(sender._subscribeToChannel).to.have.been.calledOnce
        .and.calledWith(channelStub);
      const secondCallResult = await sender._initChannel();
      expect(connectionStub.createChannel).to.have.been.calledOnce;
      expect(sender._subscribeToChannel).to.have.been.calledOnce
        .and.calledWith(channelStub);
  ...
  describe('#_subscribeToChannel', () => {
    ...
    it('should subscribe to retrun and end events', () => {
      ...
      const sender = new AMQPEventsSender(connectionStub, queueName);
      sender._subscribeToChannel(channelStub);
  ...
  describe('#disconnect', () => {
    ...
    it('should remove channel promise if failed to connect to channel', async () => {
      const sender = new AMQPEventsSender(connectionStub, queueName);
      connectionStub.createChannel = sinon.stub().returns(Promise.reject(new Error()));
      try {
        await sender._initChannel();
      } catch (e) {
        //in real code this should be procesed
      ...
      await sender.disconnect();
      expect(sender._channelPromise).to.equal(null);
    });

    it('should try to delete queue', async () => {
      const sender = new AMQPEventsSender(connectionStub, queueName);
      await sender._initChannel();
      await sender.disconnect();
      expect(channelStub.deleteQueue).to.have.been.called
        .and.calledWith(queueName);
      expect(sender._channelPromise).to.equal(null);
    });

    it('should not fail if there are no such queue', async () => {
      const sender = new AMQPEventsSender(connectionStub, queueName);
      await sender._initChannel();
      const error = new Error('there are no such queue');
      channelStub.deleteQueue = sinon.stub().returns(Promise.reject(error));
      const errorHandler = sinon.stub();
      sender.on('error', errorHandler);
      await sender.disconnect();
      expect(channelStub.deleteQueue).to.have.been.called
    ...
    it('should emit close after successfull disconnect', async () => {
      const sender = new AMQPEventsSender(connectionStub, queueName);
      await sender._initChannel();
      const closeHandler = sinon.stub();
